...
# Table 3 from paper — PatchTST/42 on ETTh1 (L=336)
# Note: PatchTST/64 (L=512) gets slightly better numbers but PatchTST/42 is the fair comparison
PAPER_SUPERVISED = {
...
# PatchTST/64 (L=512) for additional reference
PAPER_SUPERVISED_64 = {
...
# NOTE: Paper Table 4 does NOT include ETTh1 self-supervised results.
# Table 4 only covers Weather, Traffic, Electricity.
# Our ETTh1 self-supervised experiments are novel — no paper comparison available.
PAPER_SELFSUP_FT = {}  # empty — no ETTh1 self-sup in paper
PAPER_SELFSUP_LP = {}  # empty — no ETTh1 self-sup in paper
...
def generate_supervised_report(results_dir: str = "results", dataset: str = "etth1") -> None:
    ...
    paper_mse = [PAPER_RESULTS[T]["mse"] for T in horizons_found]
    ours_mae = [results[T]["test_metrics"]["mae"] for T in horizons_found]
    paper_mae = [PAPER_RESULTS[T]["mae"] for T in horizons_found]
    ...
    fig.add_trace(go.Scatter(name="Paper", x=list(PAPER_RESULTS.keys()),
                             y=[v["mse"] for v in PAPER_RESULTS.values()],
    ...
    fig.add_trace(go.Scatter(name="Paper", x=list(PAPER_RESULTS.keys()),
                             y=[v["mae"] for v in PAPER_RESULTS.values()],
                             mode="lines+markers", line=dict(color="#EF553B", width=3, dash="dash"),
                             showlegend=False), row=2, col=2)
    ...
    fig.update_layout(
        title="PatchTST Supervised Results — ETTh1",
    ...
    table_html = _build_comparison_table(results, "Supervised")

    # Write HTML
    html_content = f"""<!DOCTYPE html>
<html>
<head>
    <title>PatchTST Supervised Results — ETTh1</title>
    <style>
...
<body>
<h1>PatchTST Supervised Results — ETTh1</h1>
{table_html}
...
<html>
<head>
    <title>PatchTST Forecast Samples — ETTh1</title>
...
<body>
<h1>PatchTST Forecast Samples — ETTh1</h1>
...
def generate_selfsup_report(results_dir: str = "results", dataset: str = "etth1") -> None:
    ...
    # Paper supervised reference + our results
    # NOTE: Paper Table 4 does NOT have ETTh1 self-supervised — only our supervised baseline
    methods = [
        ("Paper Sup. (Table 3)", PAPER_SUPERVISED, "#888888", "dash"),
    ...
    fig.update_layout(
        title="PatchTST Self-Supervised Results — ETTh1",
        height=900, width=1100,
        template="plotly_white",
    )

    # Comparison table — paper only has supervised for ETTh1 (no self-supervised in Table 4)
    table_rows = ""
    for T in horizons_found:
        p_sup = PAPER_SUPERVISED[T]
        ...
        table_rows += f"""<tr>
            <td>{T}</td>
            <td>{p_sup['mse']:.3f}</td><td>{p_sup['mae']:.3f}</td>
    ...
    <table>
        <tr>
            ...
            <th colspan="2">Paper Sup. (Table 3)</th>
    ...
    <p><strong>Note:</strong> The paper (Table 4) does NOT include self-supervised results for ETTh1 —
    only Weather, Traffic, and Electricity. Our self-supervised ETTh1 experiments are <strong>novel work</strong>.
    Paper supervised (Table 3) shown for reference.</p>
    <p><em>Paper supervised from Table 3 of
    <a href="https://arxiv.org/abs/2211.14730">Nie et al. (2023)</a>, PatchTST/42 on ETTh1 with L=336.</em></p>"""

    html_content = f"""<!DOCTYPE html>
<html>
<head>
    <title>PatchTST Self-Supervised Results — ETTh1</title>
    <style>
        body {{ font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; margin: 20px; background: #fafafa; }}
        h1 {{ color: #333; }}
        table {{ border-collapse: collapse; margin: 20px 0; }}
...
<body>
<h1>PatchTST Self-Supervised Results — ETTh1</h1>
{table_html}
{fig.to_html(full_html=False, include_plotlyjs=True)}
</body>
</html>"""
...
def _build_comparison_table(results: dict, mode: str) -> str:
    ...
    for T in sorted(results.keys()):
        ...
        paper_mse = PAPER_RESULTS[T]["mse"]
        paper_mae = PAPER_RESULTS[T]["mae"]
    ...
    return f"""
    <h2>{mode} — Comparison with Paper (PatchTST/42, Table 3)</h2>
    <table>
        <tr>
            <th>Horizon</th>
    ...
    <p><em>Color: green = within 5%, yellow = 5-10%, red = >10%. Paper numbers from Table 3 of
    <a href="https://arxiv.org/abs/2211.14730">Nie et al. (2023) "A Time Series is Worth 64 Words"</a>,
    supervised PatchTST/42 on ETTh1 with L=336.</em></p>
